...
async def fil_mod(client, message): 
      ...
      m = await message.reply("**ꜱᴀᴛᴛɪɴɢ.../**")

      if args in mode_on:
          FILTER_MODE[str(message.chat.id)] = "True" 
          await m.edit("**ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ ᴇɴᴀʙʟᴇ**")
      
      elif args in mode_of:
          FILTER_MODE[str(message.chat.id)] = "False"
          await m.edit("**ᴀᴜᴛᴏ ꜰɪʟᴛᴇʀ ᴅɪꜱᴀʙʟᴇ**")
      else:
...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("ᴴᵉʸ ᶠʳᶦᵉⁿᵈ ᵖˡᵉᵃˢᵉ ˢᵉᵃʳᶜʰ ʸᵒᵘʳˢᵉˡᶠ 🫣", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
...
async def advantage_spoll_choker(bot, query):
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('ᴏᴏᴘꜱ !! ᴛʜɪꜱ ᴍᴏᴠɪᴇ ɪꜱ ɴᴏᴛ ʏᴇᴛ ʀᴇʟᴇᴀꜱᴇᴅ ᴏʀ ᴀᴅᴅᴇᴅ ᴛᴏ ᴅᴀᴛᴀʙᴀꜱᴇ 🫣')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
              InlineKeyboardButton('⛔ ᴄʟᴏꜱᴇ', callback_data='close_data'),
            InlineKeyboardButton('ʜᴏᴍᴇ 🏠', callback_data='admin')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[
            ...
            InlineKeyboardButton('𝙰𝙳𝙼𝙸𝙽', callback_data='source')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        buttons = [[  
            ...
            InlineKeyboardButton('⏺ 3/3 ⏺', callback_data='TEXT3'),
            InlineKeyboardButton('ʜᴏᴍᴇ 🏠', callback_data='deploy')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        await query.answer("ꜰᴇᴛᴄʜɪɴɢ ꜱᴇʀᴠᴇʀ ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ")
        buttons = [[
            InlineKeyboardButton('ʙᴀᴄᴋ', callback_data='admin'),
            InlineKeyboardButton('ʀᴇꜰʀᴇꜱʜ', callback_data='rfrsh')
        ]]
        ...
        if settings is not None:
            buttons = [
                [
                    ...
                    InlineKeyboardButton('ꜱɪɴɢʟᴇ' if settings["button"] else '𝐃𝐎𝐔𝐁𝐋𝐄',
                                         callback_data=f'setgs#button#{settings["button"]}#{str(grp_id)}')
                ],
                [
